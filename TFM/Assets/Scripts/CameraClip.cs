...
public class CameraClip : MonoBehaviour {
	...
	public float closestDistance = 0.5f;            // the closest distance the camera can be from the targetpublic float clipMoveTime = 0.05f;              // time taken to move when avoiding cliping (low value = fast, which it should be)
	...
	void Update() {
		...
		for (int i = 0; i < Hits.Length; i++)
		{
			...
			if (Hits[i].distance < nearest && (!Hits[i].collider.isTrigger) && Hits[i].collider.gameObject.layer == LayerMask.NameToLayer("Grounded")) {
