...
public class SceneController : MonoBehaviour {
	...
	public void FadeAndLoadSameScene(string startPosition) { //, AudioClip sceneMusic){
		...
		if (!isFading) {
			//Llamamos la corrutina para fade y cambio de escena
			StartCoroutine (FadeAndSwitchSameScene ());
		}
	}
	...
	/// <returns>The and switch scenes.</returns>
	/// <param name="sceneName">Scene name.</param>
	private IEnumerator FadeAndSwitchSameScene(){
		//Hacemos Fade Out
		yield return StartCoroutine (Fade (1f, 2f));
